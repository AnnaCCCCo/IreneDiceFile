...
local qqnum = string.sub(text, string.find(text, "\"uin\"") + 6,
    string.find(text, "}", string.find(text, "\"uin\"")) - 1)
...
if goodsname ~= "[某种谷子]" then
    if (goods ~= "yp" and goods ~= "db" and goods ~= "tt" and goods ~= "t" and goods ~= "getcode") then
        ...
        elseif (string.find(goodsname, "肾表")) then
            msg.cn = cn;
            msg.goods = goodsname;
            return "{reply_pay_money}"
        elseif (string.find(goodsname, "抽奖")) then
            local tempFile = io.open(
                "C:\\Users\\Administrator\\OneDrive\\Lists\\" .. msg.fromGroup .. "\\Goods_Group" .. msg.fromGroup ..
                    "_" .. pic .. ".csv", "r")
            local filetext = tempFile:read("*a")

            if string.find(filetext, msg.fromQQ) then
                msg.cn = cn;
                return "{reply_add_lucky_fail}"
            else
                local file = io.open(
                    "C:\\Users\\Administrator\\OneDrive\\Lists\\" .. msg.fromGroup .. "\\Goods_Group" .. msg.fromGroup ..
                        "_" .. pic .. ".csv", "a+")
                file:write(msg.fromQQ, ",", cn, ",", "\n")
                file:close()
                msg.cn = cn;
        ...
        elseif (string.find(goodsname, "排发货")) then
            ...
            local file = io.open("C:\\Users\\Administrator\\OneDrive\\Lists\\" .. msg.fromGroup .. "\\Goods_Group" ..
                                     msg.fromGroup .. "_" .. pic .. ".csv", "a+")
            file:write(os.date("%X %Y/%m/%d"), ",", msg.fromQQ, ",", cn, ",", goods, ",", "\n")
            file:close()
    ...
    elseif (goods == "yp") then
        msg.cn = cn
        msg.goodsname = goodsname
        return "{reply_add_yp}"
    elseif (goods == "db") then
        msg.cn = cn
        msg.goodsname = goodsname
        return "{reply_add_db}"
    elseif (goods == "tt" or goods == "t") then
        return "妈咪这边排~"
    end
end
